...
> **ملاحظة:**  
> إن رؤية الحاوية في حالة ”قيد التشغيل“ في بوابة Azure **لا** تعني دائمًا أنها جاهزة لمعالجة الطلبات. إذا حاولت استخدام `curl` وظهرت لك أخطاء مثل:
>
> ```text
...
> ```
>
> قد يعني ذلك أن الحاوية لم يتم تشغيلها بالمعلمات أو البيئة الصحيحة، أو أن ملفات النموذج مفقودة أو أن الاتصال بالإنترنت غير متاح لتنزيل النموذج.

### البديل: توزيع الحاوية باستخدام Azure CLI

يمكنك تجربة الأمر التالي (استبدل نقاط النهاية `rg-name` و`container-name` و`ApiKey` و`Billing` بقيمك):
...
az container create \
  ...
  --os-type Linux
```
بعد التوزيع، يمكنك اختبار نقطة النهاية (استبدل <ACI_IP> حسب الاقتضاء):
```bash
