...
اختَر أسلوبا واحدًا لإنشاء Key Vault وإضافة بيانات سرية:
...
#### استخدام Azure CLI
بدلًا من ذلك، يمكنك استخدام واجهة سطر أوامر Azure لإنشاء Key Vault وإضافة بيانات سرية.

1. في Visual Studio Code، افتح terminal.
2. قم بإنشاء Key Vault عن طريق تشغيل الأمر التالي، واستبدال `<keyVaultName>`، و`<resourceGroup>`، و`<location>` باسم Key Vault المطلوب، واسم مجموعة الموارد، ومنطقة Azure (على سبيل المثال، `eastus`):
    ...
    ```
    تضمن العلامة `--enable-rbac-authorization false` تعيين نموذج الإذن على "سياسة الوصول إلى Vault" (الافتراضي).

3. أضِف مفتاح خدمات الذكاء الاصطناعي في Azure الخاص بك كبيانات سرية في Key Vault. استبدل `<keyVaultName>` باسم Key Vault الخاص بك و`<your-key1-value>` بقيمة المفتاح 1 لخدمات الذكاء الاصطناعي في Azure الخاص بك:
...
لقد قمت الآن بإنشاء Key Vault وتخزين مفتاح خدمات الذكاء الاصطناعي في Azure الخاص بك كبيانات سرية تُسمى `AI-Services-Key`.
